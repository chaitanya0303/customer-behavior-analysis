...
select shipping_type, avg(purchase_amount) as avgpurchaseamt
from customer 
...
select subscription_status, avg(purchase_amount) as avg_spend, sum(purchase_amount) as total_rev
from customer
group by subscription_status
order by total_rev, avg_spend desc;

--Q6. Which 5 products have the highest percentage of purchases with discounts applied?
select  top 5 item_purchased , 100 * sum(case when discount_applied = 'YES' then 1 else 0 end)/ count(*) as discount
...
select itemrank, category, item_purchased, total_orders 
from item_count 
where itemrank 
<= 3;
